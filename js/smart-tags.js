...
const SmartTagGenerator = {
  ...
  processAll(images) {
    ...
    images.forEach(img => {
      ...
      // Comment: only fill if currently empty
      if (!img.metadata.comment && result.comment) {
        img.metadata.comment = result.comment;
      }

      // Title: only fill if currently empty
      if (!img.metadata.title && parsed.title) {
        img.metadata.title = parsed.title;
      }

      // Subject: only fill if currently empty
  ...
  /**
   * Extract domain and brand info from a website URL and add to image tags
   * Works with ANY website URL (not just homenest.software)
  ...
  addWebsiteTags(images, websiteUrl) {
    ...
    // Remove www. prefix for cleaner tags
    const cleanDomain = domain.replace(/^www\./, '');

    // Extract brand name from domain (e.g., "autolinkvietnam" from "autolinkvietnam.com.vn")
    const brandName = cleanDomain.split('.')[0];

    // Build website-related tags
    const websiteTags = [cleanDomain];

    // Add brand name if it's meaningful (>2 chars)
    if (brandName && brandName.length > 2) {
      websiteTags.push(brandName);
    }

    // Add full URL (without trailing slash)
    const cleanUrl = websiteUrl.replace(/\/+$/, '');
    websiteTags.push(cleanUrl);

    console.log(`[SmartTags] Adding website tags: ${websiteTags.join(', ')}`);
    ...
    images.forEach(img => {
      ...
      const existingLower = new Set(existingTags.map(t => t.toLowerCase()));
      const newTags = websiteTags.filter(t => !existingLower.has(t.toLowerCase()));

      if (newTags.length > 0) {
        img.metadata.tags = [...existingTags, ...newTags].join('; ');
      }

      // Also append domain to copyright if copyright doesn't already contain it
      if (img.metadata.copyright && !img.metadata.copyright.toLowerCase().includes(cleanDomain.toLowerCase())) {
        img.metadata.copyright = `${img.metadata.copyright} - ${cleanDomain}`;
      }
    });
  }
};
