...
const Downloader = {
  ...
  async downloadAll(images, onProgress) {
    ...
    for (const img of images) {
      try {
        ...
        // This removes Canva Program name, XMP, IPTC — everything
        let cleanDataURL = await MetadataManager.stripAllMetadata(img.dataURL);

        // Step 2: Write fresh EXIF metadata onto the clean image
        const processedDataURL = MetadataManager.writeExif(cleanDataURL, img.metadata);
  ...
  async downloadSingle(img, onProgress) {
    ...
    // Strip ALL metadata via Canvas, then write fresh EXIF
    let cleanDataURL = await MetadataManager.stripAllMetadata(img.dataURL);
    const processedDataURL = MetadataManager.writeExif(cleanDataURL, img.metadata);
